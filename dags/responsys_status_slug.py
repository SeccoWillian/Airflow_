...
def get_activated_sources():
    request = PostgresQuery().responsys_status_slug
    ...
    df.to_csv (r'/status_slug.csv', index = False, encoding="utf-8") 
...
def salvaSFTPResponsys():
    ...
    sftp = ssh.open_sftp()
    sftp.put('/status_slug.csv', '/status_slug.csv')
    ssh.close()
    

with DAG('Responsys_status_slug',
